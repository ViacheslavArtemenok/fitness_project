    ...
    <div class="offset-2 col-8">
        ...
        <form method="post" action="{{ route('admin.gyms.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="user_id">Идент пользователя : Владелец : Роль</label>
                <select class="form-control" name="user_id" id="user_id">
                    @foreach($users as $user)
                        <option value="{{ $user->id }}">{{ $user->id }} : {{ $user->name }} : {{ $roles[$user->role_id - 1]->role }}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
